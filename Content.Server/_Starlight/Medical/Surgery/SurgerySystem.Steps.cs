...
public sealed partial class SurgerySystem : SharedSurgerySystem
{
    ...
    private void OnStepOrganExtractComplete(Entity<SurgeryStepOrganExtractComponent> ent, ref SurgeryStepEvent args)
    {
        ...
        if (ent.Comp.Slot != null
            && _containers.TryGetContainer(args.Part, SharedBodySystem.GetOrganContainerId(ent.Comp.Slot),
                out var container))
        {
    ...
    private void OnStepAttachLimbComplete(Entity<SurgeryStepAttachLimbEffectComponent> _, string slot,
    ...
    private void OnStepAttachItemComplete(Entity<SurgeryStepAttachLimbEffectComponent> ent, string slot,
        ref SurgeryStepEvent args)
        => args.IsCancelled = args.Tools.Count == 0
    ...
    {
        if (TryComp(args.Body, out TransformComponent? bodyTransform) &&
            TryComp(args.Body, out HumanoidAppearanceComponent? bodyHumanoid) &&
            TryComp(args.Body, out BodyComponent? bodyComp) &&
            TryComp(args.Part, out TransformComponent? limbTransform) &&
            TryComp(args.Part, out MetaDataComponent? limbMeta) &&
            TryComp(args.Part, out BodyPartComponent? limbPart))
        {
            _limbSystem.Amputatate(
                (args.Body, bodyTransform, bodyHumanoid, bodyComp),
                (args.Part, limbTransform, limbMeta, limbPart));
        }
    }
