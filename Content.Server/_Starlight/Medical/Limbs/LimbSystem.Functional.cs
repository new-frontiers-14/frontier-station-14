...
public sealed partial class LimbSystem : SharedLimbSystem
{
    ...
    private void AddLimb(Entity<HumanoidAppearanceComponent> body, string slot, Entity<BodyPartComponent> limb)
    {
        switch (limb.Comp.PartType)
        {
            case BodyPartType.Arm:
                if (limb.Comp.Children.Keys.Count == 0)
                    _body.TryCreatePartSlot(limb,
                        limb.Comp.Symmetry == BodyPartSymmetry.Left ? "left hand" : "right hand",
                        BodyPartType.Hand, out _);
                ...
                foreach (var slotId in limb.Comp.Children.Keys)
                {
                    ...
                    foreach (var containedEnt in child.ContainedEntities)
                    {
                        ...
                        {
                            _hands.AddHand((body, hands), slotFullId,
                                limb.Comp.Symmetry == BodyPartSymmetry.Left ? HandLocation.Left : HandLocation.Right);
            ...
            case BodyPartType.Hand:
                if (TryComp<HandsComponent>(body, out var hands2))
                    _hands.AddHand((body, hands2), BodySystem.GetPartSlotContainerId(slot),
                        limb.Comp.Symmetry == BodyPartSymmetry.Left ? HandLocation.Left : HandLocation.Right);
                break;
            case BodyPartType.Leg:
                if (limb.Comp.Children.Keys.Count == 0)
                    _body.TryCreatePartSlot(limb,
                        limb.Comp.Symmetry == BodyPartSymmetry.Left ? "left foot" : "right foot",
                        BodyPartType.Foot, out _);
        ...
        foreach (var comp in EntityManager.GetComponents(limb))
        {
            ...
            {
                var interfaceEventType = typeof(LimbAttachedEvent<>).MakeGenericType(face);
                var limbAttachedInterfaceEvent = Activator.CreateInstance(interfaceEventType, [limb.Owner, comp]);

                if (limbAttachedInterfaceEvent != null)
                {
                    var closedMethod = s_raiseLocalEventRefMethod!.MakeGenericMethod(interfaceEventType);
                    closedMethod.Invoke(this, [body.Owner, limbAttachedInterfaceEvent, false]);
                }
            }
        }
    }

    private void RemoveLimb(Entity<TransformComponent, HumanoidAppearanceComponent, BodyComponent> body,
        Entity<TransformComponent, MetaDataComponent, BodyPartComponent> limb)
    {
        ...
        foreach (var comp in EntityManager.GetComponents(limb))
        {
            ...
            var limbRemovedEvent = Activator.CreateInstance(eventType, limb.Owner, comp);
            if (limbRemovedEvent != null)
            {
                var closedMethod = s_raiseLocalEventRefMethod!.MakeGenericMethod(eventType);
                closedMethod.Invoke(this, [body.Owner, limbRemovedEvent, false]);
            }

            foreach (var face in comp.GetType().GetInterfaces().Where(x => x.IsAssignableTo(typeof(IImplantable))))
            {
                var interfaceEventType = typeof(LimbRemovedEvent<>).MakeGenericType(face);
                var limbRemovedInterfaceEvent = Activator.CreateInstance(interfaceEventType, limb.Owner, comp);
                if (limbRemovedInterfaceEvent != null)
                {
                    var closedMethod = s_raiseLocalEventRefMethod!.MakeGenericMethod(interfaceEventType);
                    closedMethod.Invoke(this, [body.Owner, limbRemovedInterfaceEvent, false]);
                }
            }
        }
    }
