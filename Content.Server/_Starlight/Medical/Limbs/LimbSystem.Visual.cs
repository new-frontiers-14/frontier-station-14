...
public sealed partial class LimbSystem : SharedLimbSystem
{
    public void AddLimbVisual(Entity<HumanoidAppearanceComponent> body, Entity<BodyPartComponent> limb)
    {
        ...
        foreach (var partLimbId in _body.GetBodyPartAdjacentParts(limb, limb.Comp).Concat([limb]))
        {
                ...
                && baseLayerStorage?.Layer != null)
            {
                ...
                _humanoidAppearanceSystem.SetBaseLayerColor(body, layer.Value,
                    @base.MatchSkin ? body.Comp.SkinColor : Color.White, true, body.Comp);
            }
        }
    ...
    private void RemoveLimbVisual(Entity<TransformComponent, HumanoidAppearanceComponent, BodyComponent> body,
        Entity<TransformComponent, MetaDataComponent, BodyPartComponent> limb)
    {
        ...
        _humanoidAppearanceSystem.SetLayersVisibility(
            new Entity<HumanoidAppearanceComponent>(body.Owner, body.Comp2)!, layers, false);
    }

    public void ToggleLimbVisual(Entity<HumanoidAppearanceComponent> body,
        Entity<BaseLayerIdComponent, BaseLayerIdToggledComponent, BodyPartComponent> limb, bool toggled)
    {
        var layer = limb.Comp3.ToHumanoidLayers();
        if (layer is null)
            return;

        _humanoidAppearanceSystem.SetBaseLayerId(body, layer.Value,
            toggled ? limb.Comp2.Layer : limb.Comp1.Layer, true, body.Comp);
    }
}
