...
using Content.Shared._NF.Cargo.Prototypes;
...
public sealed partial class FrontierCargoSystem
{
    ...
    {
        SubscribeLocalEvent<FrontierCargoOrderConsoleComponent, FrontierCargoConsoleAddOrderMessage>(OnAddOrderMessage);
        SubscribeLocalEvent<FrontierCargoOrderConsoleComponent, FrontierCargoConsoleRemoveOrderMessage>(OnRemoveOrderMessage);
        SubscribeLocalEvent<FrontierCargoOrderConsoleComponent, FrontierCargoConsoleApproveOrderMessage>(OnApproveOrderMessage);
        SubscribeLocalEvent<FrontierCargoOrderConsoleComponent, BoundUIOpenedEvent>(OnOrderUIOpened);
        SubscribeLocalEvent<FrontierCargoOrderConsoleComponent, ComponentInit>(OnInit);
        Reset();
    }

    private void OnInit(EntityUid uid, FrontierCargoOrderConsoleComponent orderConsole, ComponentInit args)
    {
    ...
    private void UpdateConsole(float frameTime)
    {
        ...
        while (_timer > Delay)
        {
            ...
            {
                if (!_uiSystem.IsUiOpen(uid, FrontierCargoConsoleUiKey.Orders))
                    continue;
    ...
    private void OnApproveOrderMessage(EntityUid uid, FrontierCargoOrderConsoleComponent component,
        FrontierCargoConsoleApproveOrderMessage args)
    {
    ...
    private void OnRemoveOrderMessage(EntityUid uid, FrontierCargoOrderConsoleComponent component,
        FrontierCargoConsoleRemoveOrderMessage args)
    {
    ...
    private void OnAddOrderMessage(EntityUid uid, FrontierCargoOrderConsoleComponent component,
        FrontierCargoConsoleAddOrderMessage args)
    {
        if (args.Session.AttachedEntity is not { Valid: true } player)
            return;
        ...
        if (!HasComp<BankAccountComponent>(player) && bank == null)
            return;
        if (!TryGetOrderDatabase(uid, out var dbUid, out var orderDatabase, component))
            return;

        if (!_protoMan.TryIndex<FrontierCargoProductPrototype>(args.CargoProductId, out var product))
        {
    ...
    private void OnOrderUIOpened(EntityUid uid, FrontierCargoOrderConsoleComponent component, BoundUIOpenedEvent args)
    {
        var station = _station.GetOwningStation(uid);
    ...
    private void UpdateOrderState(FrontierCargoOrderConsoleComponent component, EntityUid? station)
    {
        if (!_uiSystem.TryGetUi(component.Owner, FrontierCargoConsoleUiKey.Orders, out var bui))
        {
            return;
        }
        ...
        if (Transform(component.Owner).GridUid is EntityUid stationGrid &&
            TryComp<BankAccountComponent>(player, out var playerBank))
        {
            station = stationGrid;
        ...
        var state = new FrontierCargoConsoleInterfaceState(
    ...
    private void PlayDenySound(EntityUid uid, FrontierCargoOrderConsoleComponent component)
    {
        _audio.PlayPvs(_audio.GetSound(component.ErrorSound), uid);
    }

    private static FrontierCargoOrderData GetOrderData(FrontierCargoConsoleAddOrderMessage args, FrontierCargoProductPrototype cargoProduct,
        int id)
    {
        return new FrontierCargoOrderData(id, cargoProduct.Product, cargoProduct.Cost, args.Amount, args.Requester,
            args.Reason);
    }

    public static int GetOutstandingOrderCount(FrontierStationCargoOrderDatabaseComponent component)
    {
        var amount = 0;
    ...
    /// </summary>
    private void UpdateOrders(EntityUid dbUid, FrontierStationCargoOrderDatabaseComponent _)
    {
        // Order added so all consoles need updating.
        var orderQuery = AllEntityQuery<FrontierCargoOrderConsoleComponent>();
    ...
    public bool AddAndApproveOrder(
        ...
        string description,
        string dest,
        FrontierStationCargoOrderDatabaseComponent component,
    ...
    )
    {
        ...
        var order = new FrontierCargoOrderData(id, spawnId, cost, qty, sender, description);
    ...
    private bool TryAddOrder(EntityUid dbUid, FrontierCargoOrderData data, FrontierStationCargoOrderDatabaseComponent component)
    {
    ...
    private static int GenerateOrderId(FrontierStationCargoOrderDatabaseComponent orderDB)
    {
    ...
    public void RemoveOrder(EntityUid dbUid, int index, FrontierStationCargoOrderDatabaseComponent orderDB)
    {
    ...
    public void ClearOrders(FrontierStationCargoOrderDatabaseComponent component)
    {
    ...
    private static bool PopFrontOrder(FrontierStationCargoOrderDatabaseComponent orderDB,
        [NotNullWhen(true)] out FrontierCargoOrderData? orderOut)
    {
    ...
    /// </summary>
    private bool FulfillNextOrder(FrontierStationCargoOrderDatabaseComponent orderDB, EntityCoordinates spawn,
        string? paperProto)
    {
    ...
    /// </summary>
    private bool FulfillOrder(FrontierCargoOrderData order, EntityCoordinates spawn, string? paperProto)
    {
    ...
    private StationBankAccountComponent? GetBankAccount(EntityUid uid, FrontierCargoOrderConsoleComponent _)
    {
        var station = _station.GetOwningStation(uid);
    ...
    private bool TryGetOrderDatabase(EntityUid uid, [MaybeNullWhen(false)] out EntityUid? dbUid,
        [MaybeNullWhen(false)] out FrontierStationCargoOrderDatabaseComponent dbComp, FrontierCargoOrderConsoleComponent _)
    {
