...
public sealed class PublicTransitSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        Enabled = _cfgManager.GetCVar(NFCVars.PublicTransit);
        FlyTime = _cfgManager.GetCVar(NFCVars.PublicTransitFlyTime);
        Counter = 0;
        StationList.Clear();
        _cfgManager.OnValueChanged(NFCVars.PublicTransit, SetTransit);
        _cfgManager.OnValueChanged(NFCVars.PublicTransitFlyTime, SetFly);
    }
    ...
    {
        base.Shutdown();
        _cfgManager.UnsubValueChanged(NFCVars.PublicTransitFlyTime, SetFly);
        _cfgManager.UnsubValueChanged(NFCVars.PublicTransit, SetTransit);
    }
        ...
        while (consoleQuery.MoveNext(out var consoleUid, out _))
        {
            if (Transform(consoleUid).GridUid == uid && TryComp(comp.NextStation, out MetaDataComponent? metadata))
            {
                ...
                _chat.TrySendInGameICMessage(consoleUid, Loc.GetString("public-transit-arrival",
                        ("destination", destinationString), ("waittime", _cfgManager.GetCVar(NFCVars.PublicTransitWaitTime))),
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var comp, out var shuttle))
        {
            ...
            comp.NextTransfer = curTime + TimeSpan.FromSeconds(FlyTime + _cfgManager.GetCVar(NFCVars.PublicTransitWaitTime));
        }
    }
    ...
    private void SetupPublicTransit()
    {
        ...
        var busMap = _cfgManager.GetCVar(NFCVars.PublicTransitBusMap);
        if (_loader.TryLoad(dummyMap, busMap, out var shuttleUids))
        {
            ...
            if (TryGetNextStation(out var station) && station is { Valid : true } destination)
            {
                ...
                transitComp.NextTransfer = _timing.CurTime + TimeSpan.FromSeconds(_cfgManager.GetCVar(NFCVars.PublicTransitWaitTime));
