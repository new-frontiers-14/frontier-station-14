...
public sealed class AutopilotSystem : EntitySystem
{
    ...
    private void OnAutopilotServerUnanchored(EntityUid uid, AutopilotServerComponent component, ref AnchorStateChangedEvent args)
    {
       ...
       // Frontier: Disables chat message from Autopilot _autopilot.SendShuttleMessage(gridUid.Value, "Autopilot: Server disconnected - autopilot disabled");
    }
    ...
    public override void Update(float frameTime)
    {
       ...
       // Frontier: Disables chat message from Autopilot _autopilot.SendShuttleMessage(uid, "Autopilot: Server lost power - autopilot disabled");
                continue;
            }
            ...
            if (distance <= autopilot.ArrivalDistance)
            {
                ...
                autopilot.Enabled = false;
              // Frontier: Disables chat message from Autopilot _autopilot.SendShuttleMessage(uid, $"Autopilot: {destinationName} reached - Parking");
    ...
    private (Vector2 avoidanceForce, float threatLevel) CalculateObstacleAvoidance(EntityUid uid, TransformComponent xform, PhysicsComponent physics, AutopilotComponent autopilot, EntityUid shuttleUid, float maxSpeed)
    {
        ...
        foreach (var grid in grids)
        {
            ...
            if (autopilot.DebugObstacles && !autopilot.ReportedObstacles.Contains(gridUid))
            {
              ...
              // Frontier: Disables chat message from Autopilot _autopilot.SendShuttleMessage(shuttleUid, $"Autopilot: Grid detected {direction} - {obstacleName} (size: {obstacleRadius * 2:F0}m, dist: {distanceToObstacle:F0}m, lateral: {lateralDistance:F0}m, combined: {combinedRadius:F0}m, inPath: {inCylinder})");
            }
    ...
    public void DisableAutopilot(EntityUid shuttleUid)
    {
     ...
     // Frontier: Disables chat message from Autopilot _autopilot.SendShuttleMessage(shuttleUid, "Autopilot: Disabled");
    }
