...
public sealed partial class CyberneticsPicker : Control
{
    ...
    public CyberneticsPicker()
    {
        ...
        Install.OnPressed += _ => InstallCybernetics();
        Remove.OnPressed += _ => RemoveCybernetics();
    ...
    {
        CollectionInstalledCybernetics = CollectionAllCybernetics
            .Where(p => cyberneticIds.Contains(p.ID))
            .ToList();
    ...
    private Dictionary<string, SpriteSpecifier.Rsi> GetIconsList()
    {
        return CollectionAllCybernetics.ToDictionary(
            ...
            p =>
            {
                if (_prototypeManager.Index(p.ID).Components.TryGetValue("Icon", out var icon)
                    && icon.Component is IconComponent iconComp)
                {
    ...
    private void InstallPart(CyberneticImplant part)
    {
        ...
        {
            var attachedImplant = CollectionAllCybernetics
                .Where(p => p.ID == attachedPart)
    ...
    {
        if (_selectedAvailableCybernetics is null
            || _selectedAvailableCybernetics.Metadata is not CyberneticImplant selectedImplant)
        {
            return;
        }
    ...
    {
        if (_selectedInstalledCybernetics is null
            || _selectedInstalledCybernetics.Metadata is not CyberneticImplant selectedImplant)
        {
            return;
        }
    ...
    public void PopulateUI()
    {
        ...
        CyberneticsPoints.Text = Loc.GetString("cybernetics-points-remaining",
            ("points", CyberneticsMaxPoints - CurrentPoints), ("points_total", CyberneticsMaxPoints));
    }
}
