...
public sealed partial class ShuttleGaslockControl : BaseShuttleControl
{
    ...
    private GaslockPortState? _viewedState;
    ...
    public GaslockConsoleBoundUserInterfaceState? DockState = null;

    private List<Entity<MapGridComponent>> _grids = new();

    private readonly HashSet<GaslockPortState> _drawnDocks = new();
    private readonly Dictionary<GaslockPortState, Button> _dockButtons = new();

    /// <summary>
    /// Store buttons for every other dock
    /// </summary>
    private readonly Dictionary<GaslockPortState, Control> _dockContainers = new();
    ...
    public ShuttleGaslockControl() : base(2f, 32f, 8f)
    {
    ...
    public void SetViewedDock(GaslockPortState? dockState)
    {
    ...
    protected override void Draw(DrawingHandleScreen handle)
    {
        ...
        _grids.Clear();
        _mapManager.FindGridsIntersecting(gridXform.MapID,
        ...
        var viewedDockType = _viewedState?.DockType ?? DockType.None;
        var viewedReceiveOnly = _viewedState?.ReceiveOnly ?? true;
        ...
        foreach (var grid in _grids)
        {
            ...
            if (!DockState.State.Docks.TryGetValue(EntManager.GetNetEntity(grid), out var gridDocks))
                continue;
            ...
            foreach (var dock in gridDocks)
            {
                ...
                otherDockColor = Color.ToSrgb(HighlightedDock == dock.Entity
                    ? dock.HighlightedRadarColor
                    : dock.RadarColor);
                ...
                if (viewedDockPos != null && dock.Coordinates.NetEntity != gridNent)
                {
                    ...
                    if (range.Length() < SharedDockingSystem.DockingHiglightRange * MinimapScale)
                    {
                        if (_viewedState?.GridDockedWith == null)
                        {
                            ...
                            {
                                dockButton.Disabled =
                                    !canDock && dock.GridDockedWith == null ||
                                    !canDockChange;
                                dockButton.Visible = dock.GridDockedWith != null ||
    ...
    public void BuildDocks(EntityUid? shuttle)
    {
        ...
        foreach (var (otherShuttle, docks) in DockState.State.Docks)
        {
