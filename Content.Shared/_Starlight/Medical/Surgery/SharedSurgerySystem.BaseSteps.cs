...
public abstract partial class SharedSurgerySystem
{
    ...
    private void InitializeSteps()
    {
        ...
        Subs.BuiEvents<SurgeryTargetComponent>(SurgeryUIKey.Key,
            subs => subs.Event<SurgeryStepChosenBuiMsg>(OnSurgeryTargetStepChosen));
    }
    ...
    private void OnTargetDoAfter(Entity<SurgeryTargetComponent> ent, ref SurgeryDoAfterEvent args)
    {
        ...
        if (!_random.Prob(args.SuccessRate))
        {
            if (_net.IsClient)
                return;
            _popup.PopupEntity(
                "Because of a careless tool, your hand shook. You need to start this step all over again!",
                args.User,
    ...
    private void OnCanPerformStep(Entity<SurgeryStepComponent> ent, ref SurgeryCanPerformStepEvent args)
    {
        ...
        foreach (var reg in ent.Comp.Tools.Values)
        {
            ...
            if (TryComp<SurgeryItemSizeConditionComponent>(ent, out var itemSizeComp)
                && TryComp<ItemComponent>(tool, out var item)
                && _item.GetSizePrototype(item.Size) > _item.GetSizePrototype(itemSizeComp.Size))
            {
    ...
    public bool PreviousStepsComplete(EntityUid body, EntityUid part, Entity<SurgeryComponent> surgery, EntProtoId step)
    {
        ...
        foreach (var surgeryStep in surgery.Comp.Steps)
        {
            ...
            if (Prototype(surgery.Owner) is not EntityPrototype surgProto
                || !IsStepComplete(part, surgProto.ID, surgeryStep))
                return false;
        }
