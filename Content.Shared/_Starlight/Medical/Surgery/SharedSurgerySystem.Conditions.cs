...
public abstract partial class SharedSurgerySystem
{
    ...
    private void OnOrganDontExistConditionValid(Entity<SurgeryOrganDontExistConditionComponent> ent,
    ...
    private void OnOrganExistConditionValid(Entity<SurgeryOrganExistConditionComponent> ent, ref SurgeryValidEvent args)
    {
        ...
        if (ent.Comp.Container != null)
        {
            foreach (var slotId in Comp<BodyPartComponent>(mainPart).Organs.Keys)
            {
                if (ent.Comp.Container == slotId)
                {
                    if (!_containers.TryGetContainer(mainPart, SharedBodySystem.GetOrganContainerId(ent.Comp.Container),
                            out var container))
                        continue;
    ...
    private void OnPartConditionValid(Entity<SurgeryPartConditionComponent> ent, ref SurgeryValidEvent args)
    {
        ...
        if (TryComp<BodyPartComponent>(args.Body, out var itemPart)
            && itemPart.PartType is BodyPartType item
            && !ent.Comp.Parts.Contains(item))
        {
            Logger.Warning("don't have part at part");
            args.Cancelled = true;
        }

        if (CompOrNull<BodyPartComponent>(args.Part)?.PartType is BodyPartType part
            && !ent.Comp.Parts.Contains(part))
            args.Cancelled = true;
    }
    ...
    private void OnSpeciesConditionValid(Entity<SurgerySpeciesConditionComponent> ent, ref SurgeryValidEvent args)
    {
        ...
        if (ent.Comp.SpeciesWhitelist.Count > 0
            && !ent.Comp.SpeciesWhitelist.Contains(humanoidAppearanceComponent.Species))
        {
            args.Cancelled = true;
            return;
        }
    ...
    private void OnAnyLimbSlotConditionValid(Entity<SurgeryAnyLimbSlotConditionComponent> ent,
        ref SurgeryValidEvent args)
    {
    ...
    private void OnLimbSlotConditionValid(Entity<SurgeryLimbSlotConditionComponent> ent, ref SurgeryValidEvent args)
        => args.Cancelled = !(_containers.TryGetContainer(args.Part,
                SharedBodySystem.GetPartSlotContainerId(ent.Comp.Slot), out var container)
