...
public sealed class PatchSystem : EntitySystem
{
    ...
    private bool TryApply(EntityUid patch, PatchComponent patchComponent, EntityUid user, EntityUid target)
    {
        ...
        _audio.PlayPvs(patchComponent.ApplyBeginSound, patch,
        ...
        if (isNotSelf)
        {
            var msg = Loc.GetString("medical-item-popup-target",
                ("user", Identity.Entity(user, EntityManager)), ("item", patch));
            _popupSystem.PopupEntity(msg, target, target, PopupType.Medium);
        }

        var delay = isNotSelf ? patchComponent.Delay : patchComponent.Delay; // TODO: apply self-use penalties
    ...
    private void OnDoAfter(Entity<DamageableComponent> entity, ref PatchDoAfterEvent args)
    {
        ...
        if (_solutionContainerSystem.ResolveSolution(patch, patchComponent.SolutionContainer, ref solutionEntity,
                out var solution))
        {
            foreach (var reagent in solution.Contents)
                patchUser.ReagentsToInsert.Add(reagent);
        }
        ...
        while (query.MoveNext(out var uid, out var patchUser))
        {
                ...
                && injectableSolution != null)
            {
                for (var i = patchUser.ReagentsToInsert.Count - 1; i >= 0; i--)
                {
                    ...
                    {
                        reagent = new ReagentQuantity(reagent.Reagent, reagent.Quantity - quantityToInsert);
